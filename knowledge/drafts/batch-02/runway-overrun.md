# Runway Overrun

**Catalog ID:** 0052  
**Category:** Airport Operations  
**Draft:** Premium batch-02 — working document (not consumed by Captain)

## Meaning

Aircraft passing beyond the end of the runway during landing or rejected takeoff.

## Operational Meaning

Serious runway safety event; crew reports position; runway closed for inspection.

## When Used

Late braking, wet runway, or high approach speed causes aircraft to exceed runway end.

## Who Uses It

Pilots report to Tower; ATC alerts emergency services and closes runway.

## Real ATC Phraseology

- RESEARCH REQUIRED
- RESEARCH REQUIRED
- RESEARCH REQUIRED
- RESEARCH REQUIRED
- RESEARCH REQUIRED

## Real Pilot Readbacks

- RESEARCH REQUIRED
- RESEARCH REQUIRED
- RESEARCH REQUIRED
- RESEARCH REQUIRED
- RESEARCH REQUIRED

## Common ICAO Speaking Questions

- What is the difference between runway overrun and runway excursion?
- What factors would you report to ATC after an overrun?
- How does runway condition affect overrun risk?

## Common Brazilian Mistakes

- Confusing overrun with go-around.
- RESEARCH REQUIRED
- RESEARCH REQUIRED

## Pronunciation Coaching

- Overrun — O-ver-run.
- Threshold — THRESH-old.
- Braking — BRAKE-ing.

## Captain Teaching Notes

Part 3 discussion concept — no verified ICAO Delta phraseology yet; use SKYbrary for operational meaning only.

## Memory Trick

END — Evaluate speed, Needed distance, Decide go-around early.

## Related Concepts

- Runway Excursion
- Rejected Takeoff
- Go Around
- Runway Incursion

## References

- [SKYbrary — Runway Overrun](https://skybrary.aero/articles/runway-overrun)
- [FAA Pilot/Controller Glossary](https://www.faa.gov/air_traffic/publications/atpubs/pcg_html/glossary.html)
- [FAA AIM — Runway Safety](https://www.faa.gov/air_traffic/publications/atpubs/aim_html/chap4_section_3.html)
