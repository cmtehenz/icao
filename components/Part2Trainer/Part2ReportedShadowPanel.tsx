...
export default function Part2ReportedShadowPanel({
  audioSrc,
  audioLabel,
  modelReported,
  context,
  ...
  const startShadow = async () => {
    ...
    try {
      ...
      setPhase("recording");
      await azure.startRecording(modelReported);
    } catch {
      setPhase("idle");
      setNote("Não foi possível tocar o áudio do controller. Verifique os arquivos de prova.");
    }
  };
  ...
  const stopShadow = async () => {
    ...
    if (assessment) {
      const ctx = {
        ...
        situationId,
        part2MissionKind: "reported" as const,
      };
      tryMarkPart2DailyMissionPractice({
        part2MissionKind: "reported",
        situationId,
        accuracy,
        recognizedText: assessment.recognizedText,
      });
    ...
    <div className={`peel-shadowing-panel part2-readback-shadow ${embedded ? "embedded" : ""}`}>
      {!embedded && (
        <div className="peel-shadowing-head">
          <h3>Shadow reported — ouvir controller → reported speech</h3>
      ...
      {!azure.configured ? (
        <p className="voice-coach-warn">Configure Azure Speech para avaliar o reported speech.</p>
      ) : (
        <p className="peel-shadowing-sub">
          Ouça o que o controller disse ({audioLabel}) → espere 1s → responda em reported speech →
          nota Azure.
        </p>
      )}
      ...
      <p className="part2-readback-shadow-model">{modelReported}</p>

      {note && <p className="voice-coach-warn">{note}</p>}

      <div className="peel-shadowing-active">
        <p className="peel-shadowing-phase">
          {phase === "listening" && "🔊 Ouvindo o controller…"}
          {phase === "waiting" && "⏳ Prepare o reported speech…"}
          {phase === "recording" && "● Reporte o que o controller disse"}
          ...
          <div className="peel-shadowing-result">
            ...
            {score.accuracy < SHADOW_PEEL_PASS_SCORE && (
              <p className="peel-shadowing-tip">
                Use past tense e third person — fale devagar para o Azure ouvir tudo.
              </p>
            )}
            {score.heard && (
            ...
            <AudioCompareReplay
              ...
              modelText={modelReported}
              userAudioBlob={lastAudioBlob}
              modelLabel="Controller"
              userLabel="Você"
            />
